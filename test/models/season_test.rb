...
class SeasonTest < ActiveSupport::TestCase
  ...
  it 'adds a member' do
    ...
    season_two.members.add_member(@first_user).must_equal true
    season_two.users.reload
  ...
  #   simple_season.max_members = 1
  #   simple_season.save.must_equal true
  #   simple_season.members.add_member(users(:one)).must_equal true
  #   simple_season.members.add_member(users(:two)).wont_equal true
  ...
  it 'is idempotent with saving the same event member' do
    assert_no_difference('@season_one.members.size') do
      success = @season_one.members.add_member(@first_user, admin_flag: false)
