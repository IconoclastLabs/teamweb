module NavigationHelper
    ...
    def render_breadcrumbs
        ...
        navigation_add @coordinator.name, coordinator_path(@coordinator) if defined?(@coordinator) && @coordinator.name
        navigation_add "Events", coordinator_events_path(@coordinator)  if @events || @event
        navigation_add @event.name, coordinator_event_path(@event.coordinator, @event) if defined?(@event) && @event.name
        navigation_add "Teams", coordinator_event_teams_path(@coordinator, @event) if @teams || @team
        navigation_add @team.name, coordinator_event_team_path(@coordinator, @event, @team) if defined?(@team) && @team.name
